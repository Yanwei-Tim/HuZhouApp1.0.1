...
import com.database.pojo.CategorysTable;
...
public final class DataParser {
            ...
            for (int i = 0; i < nameList.size(); i++) //循环解析所有的记录
            {
                if (i % recordNum == 0) {
                    ...
                    for (String documentId : documentIdList.get(recordIndex)) {
                        ...
                        tableData.getAccessaryFileList().add(fileUrl);
                    }
                } else {
                    tableData.getRecord().putField(nameList.get(i), valueList.get(i));
    ...
    public static BaseTable newTableInstance(String tableName) {
        ...
        switch (tableName) {
            case AlbumTable.TABLE_NAME:
                tableData = new AlbumTable();
                break;
            case ContentTable.TABLE_NAME:
                tableData = new ContentTable();
                break;
            case CourseTable.TABLE_NAME:
                tableData = new CourseTable();
                break;
            case EnquiryTable.TABLE_NAME:
                tableData = new EnquiryTable();
                break;
            case ExpertTable.TABLE_NAME:
                tableData = new ExpertTable();
                break;
            case NewsTable.TABLE_NAME:
                tableData = new NewsTable();
                break;
            case OpinionTable.TABLE_NAME:
                tableData = new OpinionTable();
                break;
            case ReplyTable.TABLE_NAME:
                tableData = new ReplyTable();
                break;
            case StudyInfoTable.TABLE_NAME:
                tableData = new StudyInfoTable();
                break;
            case StudyScoreTable.TABLE_NAME:
                tableData = new StudyScoreTable();
                break;
            case ThumbsTable.TABLE_NAME:
                tableData = new ThumbsTable();
                break;
            case UserInfoTable.TABLE_NAME:
                tableData = new UserInfoTable();
                break;
            case UserTable.TABLE_NAME:
                tableData = new UserTable();
                break;
            case CategorysTable.TABLE_NAME:
                tableData = new CategorysTable();
                break;

            default:
                tableData = new BaseTable();
                break;
        }
