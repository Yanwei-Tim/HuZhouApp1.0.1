    ...
    public static ArrayList<Expert> queryExpertList() throws Exception {
        ...
        if (expertTableList != null) {
            for (ExpertTable expertTable : expertTableList) {
                ...
                if (userData != null)
                    uIList = (List<UserInfoTable>) DataOperation.queryTable(UserInfoTable.TABLE_NAME, UserInfoTable.FIELD_USERID, userData.getContentId());
                if (uIList != null && uIList.size() != 0)
                    userInfoData = (UserInfoTable) DataOperation.queryTable(UserInfoTable.TABLE_NAME, UserInfoTable.FIELD_USERID, userData.getContentId()).get(0);
