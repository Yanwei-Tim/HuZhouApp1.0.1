...
public final class DataOperationHelper
{
    ...
    public static ArrayList<Expert> queryExpertList(String expertCategoryId) throws Exception
    {
        ...
        //专家Table列表
        List<ExpertTable> expertTableList;
        expertTableList = (List<ExpertTable>) DataOperation.queryTable(ExpertTable.TABLE_NAME, ExpertTable.FIELD_CATEGORYID, expertCategoryId);
        if(expertTableList!=null)
        {
            for (ExpertTable expertTable : expertTableList)
            {
    ...
    public static ArrayList<Question> queryUserAnswerQuestionList(UserTable user, int firstIndex, int num) throws Exception
    {
    ...
    public static ArrayList<Question> queryUserAskQuestionList(UserTable user, int firstIndex, int num) throws Exception
    {
        ArrayList<Question> questionList = new ArrayList<>();

        //当前用户的问题列表
        List<EnquiryTable> askList = (ArrayList<EnquiryTable>)DataOperation.queryTable(EnquiryTable.TABLE_NAME, EnquiryTable.FIELD_USERNO, firstIndex, num,user.getContentId()); //用户提问
    ...
    public static Question uploadQuestion(UserTable asker, String content, String time, String category) throws Exception
    {
        ...
        enquiryData.putField(EnquiryTable.FIELD_APPLYTIME, new SimpleDateFormat(com.geekband.huzhouapp.utils.Constants.TIME_FORMAT).format(new Date(System.currentTimeMillis())));
